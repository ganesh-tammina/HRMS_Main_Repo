...
@Component({
  ...
  template: `
    <div class="ion-text-left">
      ...
      <div class="ms-2" *ngIf="temp && currentUrl == '/Me'">
        Since Last Login :
...
export class ClockButtonComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    if (this.routeGaurdService.employeeID) {
      const actualRecord = this.attendanceService.getRecord(
        Number(this.routeGaurdService.employeeID)
      );
      if (actualRecord) {
  ...
  ngOnInit() {
    ...
    if (
      this.routeGaurdService.token &&
      this.routeGaurdService.refreshToken &&
      ...
      this.attendanceService
        .checkLoginOrLoggedOut(this.routeGaurdService.employeeID)
        .subscribe((res: any) => {
          if (res && res.isIn) {
            this.temp = res.isIn != null ? res.isIn : res.isIn;
          }
        });
    } else if (
      this.currentCandidate &&
  ...
  clockIn() {
    ...
    this.attendanceService.responseSubject.next({
      action: 'in',
      optimistic: true,
    });
    ...
    this.attendanceService.clockInServer(clockInData).subscribe({
      ...
      error: (err) => {
        ...
        if (errorMsg.includes('Already Clocked In')) {
          console.log(
            'User already clocked in on server, updating local state...'
          );
          if (this.record) {
  ...
  clockOut() {
    ...
    this.attendanceService.clockOutServer(clockOutData).subscribe({
      ...
      error: (err) => {
        ...
        const errorMsg =
          err.error?.message || err.message || 'Clock-out failed';
        alert(errorMsg);
      },
    });
  }
  ...
  private checkServerAttendanceStatus(empId: number) {
    console.log('🔍 Checking server attendance status for employee:', empId);
    ...
    this.attendanceService
      ...
      .subscribe({
        next: (att) => {
          ...
          console.log('📊 Server attendance data:', data);

          let isClockedIn = false;
          let lastClockInTime: string | undefined;

          if (data && data.attendance && data.attendance.length > 0) {
            // Sort by time to get the latest entry
            const sortedRecords = data.attendance.sort((a: any, b: any) => {
            ...
            isClockedIn = latestRecord.check_in && !latestRecord.check_out;

            if (isClockedIn && latestRecord.check_in) {
              lastClockInTime = currentDate + 'T' + latestRecord.check_in;
            }
          }
          ...
          console.log('✅ Updated clock status from server:', {
            isClockedIn,
            lastClockInTime,
          });
        },
