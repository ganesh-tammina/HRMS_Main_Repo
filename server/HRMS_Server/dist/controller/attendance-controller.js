"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const attendance_service_1 = __importDefault(require("../services/attendance-service"));
class AttendanceController {
    static async handleClockIn(req, res) {
        const { employee_id, check_in } = req.body;
        if (!employee_id) {
            return res.status(400).json({ message: 'employee_id is required' });
        }
        if (typeof employee_id != 'number') {
            return res.status(400).json({ message: 'Not Valid Employee Number' });
        }
        const response = await attendance_service_1.default.countLogin({
            employee_id,
            check_in,
        });
        if (response.number == 0) {
            try {
                const result = await attendance_service_1.default.clockIn({
                    employee_id,
                    check_in,
                });
                res.json({ message: 'Clock-in successful', data: result });
            }
            catch (err) {
                console.error(err);
                res.status(500).json({ message: 'Internal server error' });
            }
        }
        else {
            res.status(400).json({ message: 'Already Logged In.' });
        }
    }
    static async handleClockOut(req, res) {
        const { employee_id, check_out } = req.body;
        if (!employee_id) {
            return res.status(400).json({ message: 'employee_id is required' });
        }
        if (typeof employee_id != 'number') {
            return res.status(400).json({ message: 'Not Valid Employee Number' });
        }
        try {
            const result = await attendance_service_1.default.clockOut({
                employee_id,
                check_out,
            });
            res.json({ message: 'Clock-Out successful', data: result });
        }
        catch (err) {
            console.error(err);
            res.status(500).json({ message: 'Internal server error' });
        }
    }
    static async getAttendance(req, res) {
        if (!req.body.employee_id) {
            return res.status(400).json({ message: 'employee_id is required' });
        }
        else {
            let result;
            if (req.body.startDate && req.body.endDate) {
                result = await attendance_service_1.default.getAttendance({
                    employee_id: req.body.employee_id,
                    startDate: req.body.startDate,
                    endDate: req.body.endDate,
                });
            }
            else if (req.body.date) {
                result = await attendance_service_1.default.getTodayAttendanceExtra(req.body.employee_id, req.body.date);
            }
            else {
                result = await attendance_service_1.default.getAttendance({
                    employee_id: req.body.employee_id,
                });
            }
            res.status(200).json({ attendance: result });
        }
    }
    static async notinyet(req, res) {
        const result = await attendance_service_1.default.notinyet();
        res.json({
            not_in: result,
            count: result.length,
        });
    }
    static async upsertAttendanceRecord(req, res) {
        try {
            const result = await attendance_service_1.default.upsertAttendanceRecord(req.body);
            res.status(200).json({
                success: true,
                message: result.message,
                recordId: result.recordId,
            });
        }
        catch (error) {
            console.error('[AttendanceController] upsertAttendanceRecord error:', error);
            res
                .status(500)
                .json({ success: false, error: 'Failed to upsert attendance record' });
        }
    }
    static async addAttendanceEvent(req, res) {
        try {
            const result = await attendance_service_1.default.addAttendanceEvent(req.body);
            res.status(200).json({
                success: true,
                message: result.message,
                eventId: result.eventId,
            });
        }
        catch (error) {
            console.error('[AttendanceController] addAttendanceEvent error:', error);
            res
                .status(500)
                .json({ success: false, error: 'Failed to add attendance event' });
        }
    }
    static async upsertDailyAccumulation(req, res) {
        try {
            const result = await attendance_service_1.default.upsertDailyAccumulation(req.body);
            res.status(200).json({ success: true, message: result.message });
        }
        catch (error) {
            console.error('[AttendanceController] upsertDailyAccumulation error:', error);
            res
                .status(500)
                .json({ success: false, error: 'Failed to upsert daily accumulation' });
        }
    }
    static async getEmployeeAttendance(req, res) {
        try {
            if (!req.params.employeeId) {
                res
                    .status(400)
                    .json({ success: false, error: 'employeeId parameter is required' });
                return;
            }
            const employeeId = parseInt(req.params.employeeId, 10);
            if (isNaN(employeeId)) {
                res
                    .status(400)
                    .json({ success: false, error: 'Invalid employeeId parameter' });
                return;
            }
            const data = await attendance_service_1.default.getEmployeeAttendance(employeeId);
            if (!data) {
                res.status(404).json({
                    success: false,
                    message: 'No attendance found for this employee',
                });
                return;
            }
            res.status(200).json({ success: true, data });
        }
        catch (error) {
            console.error('[AttendanceController] getEmployeeAttendance error:', error);
            res
                .status(500)
                .json({ success: false, error: 'Failed to fetch attendance data' });
        }
    }
    static async llakdjlfjas(reeq, res) {
        if (!reeq.body?.employee_id) {
            return res.status(400).json({ message: 'employee_id is required' });
        }
        const resu = await attendance_service_1.default.getTodayAttendance(reeq.body.employee_id);
        if (resu.length === 0) {
            res.status(200).json({ message: 'No Attendance Found' });
        }
        res.status(200).json({
            clockin: resu[0].check_in != null ? true : false,
            clockout: resu[0].check_out != null ? true : false,
            attendance_id: resu[0].attendance_id,
            employee_id: resu[0].employee_id,
            attendance_date: new Date(resu[0].attendance_date).toLocaleDateString(),
        });
    }
    static async kasdja(req, res) {
        try {
            const { LogType, EmpID } = req.body;
            const currentTime = String(new Date().toTimeString().split(' ')[0]);
            const currentDate = String(new Date().toISOString().split('T')[0]);
            const todayRecords = await attendance_service_1.default.getAttendance({
                employee_id: EmpID,
                date: currentDate,
            });
            const { shift_policy_name } = await attendance_service_1.default.qeiwoi(EmpID);
            if (!shift_policy_name) {
                return res.status(400).json({
                    status: false,
                    message: 'No Shift Policy Assigned, contact administrator',
                });
            }
            const shiftPolicy = await attendance_service_1.default.qeiwoasi(shift_policy_name);
            if (!shiftPolicy) {
                return res.status(400).json({
                    status: false,
                    message: 'No Shift Policy, contact administrator',
                });
            }
            if (LogType === 'IN') {
                const hasOpenSession = todayRecords.some((rec) => rec.check_in && !rec.check_out);
                if (hasOpenSession) {
                    return res.status(400).json({
                        status: false,
                        message: 'Already Clocked In — please Clock Out first',
                    });
                }
                const clockInResult = await attendance_service_1.default.clockIn({
                    employee_id: EmpID,
                    check_in: currentTime,
                });
                const isLate = currentTime > shiftPolicy.check_in;
                const timeDiff = AttendanceController.diff
                    ? AttendanceController.diff(currentTime, shiftPolicy.check_in)
                    : null;
                return res.status(200).json({
                    status: true,
                    message: 'Clocked In Successfully',
                    currentDate: currentDate,
                    late: {
                        shift_check_in: shiftPolicy.check_in,
                        actual_check_in: currentTime,
                        is_late: isLate,
                        diff: timeDiff,
                    },
                    data: clockInResult,
                });
            }
            if (LogType === 'OUT') {
                const todayRecords = await attendance_service_1.default.getAttendance({
                    employee_id: EmpID,
                    date: currentDate,
                });
                if (!todayRecords || todayRecords.length === 0) {
                    return res.status(400).json({
                        status: false,
                        message: 'You are not clocked in today',
                    });
                }
                const openSession = todayRecords.find((rec) => rec.check_in && !rec.check_out);
                if (!openSession) {
                    return res.status(400).json({
                        status: false,
                        message: 'You are not clocked in today',
                    });
                }
                const clockOutResult = await attendance_service_1.default.clockOut({
                    employee_id: EmpID,
                    check_out: currentTime,
                });
                if (clockOutResult.status === false) {
                    return res.status(400).json(clockOutResult);
                }
                return res.status(200).json({
                    status: true,
                    message: 'Clocked Out Successfully',
                    currentDate: currentDate,
                    data: clockOutResult,
                    shift_check_out: shiftPolicy.check_out,
                });
            }
            return res
                .status(400)
                .json({ status: false, message: 'Invalid LogType' });
        }
        catch (err) {
            console.error('Error in kasdja:', err);
            return res.status(500).json({
                status: false,
                message: 'Server Error',
                error: err.message,
            });
        }
    }
    static diff(time1, time2) {
        const [h1, m1, s1] = time1.split(':').map(Number);
        const [h2, m2, s2] = time2.split(':').map(Number);
        const date1 = new Date();
        date1.setHours(h1, m1, s1, 0);
        const date2 = new Date();
        date2.setHours(h2, m2, s2, 0);
        let diffMs = Math.abs(date1.getTime() - date2.getTime());
        const hours = Math.floor(diffMs / (1000 * 60 * 60));
        diffMs -= hours * 1000 * 60 * 60;
        const minutes = Math.floor(diffMs / (1000 * 60));
        diffMs -= minutes * 1000 * 60;
        const seconds = Math.floor(diffMs / 1000);
        const pad = (n) => n.toString().padStart(2, '0');
        return `${pad(hours)}:${pad(minutes)}:${pad(seconds)}`;
    }
}
exports.default = AttendanceController;
//# sourceMappingURL=attendance-controller.js.map