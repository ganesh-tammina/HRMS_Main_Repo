...
class Employeeservices {
    async viewEmployement_details(req, res) {
...
`;
        const [result] = req?.body?.id
            ? await database_1.pool.query(getAllUsingID, [req.body.id])
            : await database_1.pool.query(getAllQUery);
        return {
            success: true,
            statusCode: 200,
            message: 'Yep',
    ...
    async addEmployees(req, res, standalone) {
        let connection = null;
        try {
            const { EmployeeNumber, FirstName, MiddleName, LastName, FullName, WorkEmail, Gender, MaritalStatus, BloodGroup, PhysicallyHandicapped, Nationality, } = req.body;
            if (!EmployeeNumber || !WorkEmail) {
                return {
                    success: false,
                    statusCode: 400,
                    message: 'Missing required fields: EmployeeNumber and WorkEmail are mandatory.',
            ...
            const queryConnection = standalone ? connection : database_1.pool;
            const created_at = new Date();
            const updated_at = new Date();
            if (queryConnection) {
                const [result] = await queryConnection.query(`INSERT INTO employees 
    ...
    async addAddress(req, res, addType, standalone) {
        let connection = null;
        try {
            ...
            if (queryConnection) {
                const [result] = await queryConnection.query(`INSERT INTO addresses 
    ...
    async addEmployement_details(req, res, standalone) {
        let connection = null;
        try {
            const data = req.body;
            const { employee_id, AttendanceNumber, Location, LocationCountry, LegalEntity, BusinessUnit, Department, SubDepartment, JobTitle, SecondaryJobTitle, ReportingTo, ReportingManagerEmployeeNumber, DottedLineManager, DateJoined, LeavePlan, Band, PayGrade, TimeType, WorkerType, ShiftPolicyName, WeeklyOffPolicyName, AttendanceTimeTrackingPolicy, AttendanceCaptureScheme, HolidayListName, ExpensePolicyName, NoticePeriod, CostCenter, } = data;
            if (!employee_id) {
                return {
                    success: false,
                    statusCode: 400,
            ...
            const queryConnection = standalone ? connection : database_1.pool;
            if (queryConnection) {
                const [result] = await queryConnection.query(`INSERT INTO employment_details 
    ...
    async addExitdetails(req, res, standalone) {
        let connection = null;
        try {
            const { employee_id, EmploymentStatus, ExitDate, Comments, ExitStatus, TerminationType, TerminationReason, ResignationNote, } = req.body;
            if (!employee_id) {
                return {
                    success: false,
                    statusCode: 400,
            ...
            const queryConnection = standalone ? connection : database_1.pool;
            if (queryConnection) {
                const [result] = await queryConnection.query(`INSERT INTO exit_details 
    ...
    async addFamilyInfo(req, res, standalone) {
        let connection = null;
        try {
            const { employee_id, FatherName, MotherName, SpouseName, ChildrenNames } = req.body;
            if (!employee_id) {
                return {
                    success: false,
                    statusCode: 400,
            ...
            const queryConnection = standalone ? connection : database_1.pool;
            if (queryConnection) {
                const [result] = await queryConnection.query(`INSERT INTO family_info 
    ...
    async addStatutoryInfo(req, res, standalone) {
        let connection = null;
        try {
            const { employee_id, PANNumber, AadhaarNumber, PFNumber, UANNumber } = req.body;
            if (!employee_id) {
                return {
                    success: false,
                    statusCode: 400,
            ...
            const queryConnection = standalone ? connection : database_1.pool;
            if (queryConnection) {
                const [result] = await queryConnection.query(`INSERT INTO statutory_info 
    ...
    async viewEmployees(req, res) {
        let conn = null;
        try {
            conn = await database_1.pool.getConnection();
    ...
    async editEmployees(req, res) {
        ...
        try {
            ...
            const [result] = await conn.query(`UPDATE employees SET ${setClause} WHERE employee_id = ?`, values);
    ...
    async deleteEmployees(req, res) {
        ...
        try {
            ...
            const [result] = await conn.query('DELETE FROM employees WHERE employee_id = ?', [employee_id]);
            if (result.affectedRows === 0) {
                throw new Error('Employee not found');
            }
            await conn.commit();
    ...
    async editAddress(req, res) {
        ...
        try {
            ...
            const setClause = fields.map((f) => `${f} = ?`).join(', ');
            const values = fields.map((f) => updates[f] != null ? this.truncateZip(updates[f]) : null);
            values.push(employee_id, address_type);
            const [result] = await conn.query(`UPDATE addresses SET ${setClause} WHERE employee_id = ? AND address_type = ?`, values);
            if (result.affectedRows === 0) {
                await conn.query(`INSERT INTO addresses (employee_id, address_type, address_line1, address_line2, city, state, zip, country)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [employee_id, address_type, ...values.slice(0, 6)]);
            }
            await conn.commit();
    ...
    async deleteAddress(req, res) {
        ...
        try {
            ...
            conn = await database_1.pool.getConnection();
            const [result] = await conn.query('DELETE FROM addresses WHERE employee_id = ? AND address_type = ?', [employee_id, address_type]);
            return {
                success: true,
                message: result.affectedRows > 0 ? 'Address deleted' : 'Address not found',
                statusCode: 200,
            };
        }
        catch (error) {
    ...
    async editEmployement_details(req, res) {
        let conn = null;
        try {
            const { employee_id, ...updates } = req.body;
            if (!employee_id)
            ...
            const reportingMgrNum = updates.reporting_manager_employee_number
                ? await this.getEmployeeIdByNumber(conn, this.resolveReportingManager(updates.reporting_manager_employee_number))
                : null;
            const setClause = allowed.map((f) => `${f} = ?`).join(', ');
            const values = allowed.map((f) => {
                if (f === 'date_joined')
                    return this.formatDate(updates[f]);
                if (f === 'notice_period')
                    return this.parseNoticePeriod(updates[f]);
                if (f === 'reporting_manager_employee_number')
                    return reportingMgrNum ? updates[f] : null;
                return updates[f] ?? null;
            });
            values.push(employee_id);
            const [result] = await conn.query(`UPDATE employment_details SET ${setClause} WHERE employee_id = ?`, values);
            if (result.affectedRows === 0) {
    ...
    async deleteEmployement_details(req, res) {
        let conn = null;
        try {
            const { employee_id } = req.body;
            if (!employee_id)
            ...
            conn = await database_1.pool.getConnection();
            const [result] = await conn.query('DELETE FROM employment_details WHERE employee_id = ?', [employee_id]);
            return {
                success: true,
    ...
    async viewExitdetails(req, res) {
        ...
        try {
            ...
            conn = await database_1.pool.getConnection();
            const [rows] = await conn.query('SELECT * FROM exit_details WHERE employee_id = ?', [employee_id]);
            return {
                success: true,
    ...
    async editExitdetails(req, res) {
        ...
        try {
            ...
            const setClause = fields.map((f) => `${f} = ?`).join(', ');
            const values = fields.map((f) => f === 'exit_date' ? this.formatDate(updates[f]) : updates[f] ?? null);
            values.push(employee_id);
            const [result] = await conn.query(`UPDATE exit_details SET ${setClause} WHERE employee_id = ?`, values);
            if (result.affectedRows === 0) {
                await conn.query(`INSERT INTO exit_details (employee_id, ${fields.join(', ')})
           VALUES (?, ${fields.map(() => '?').join(', ')})`, [employee_id, ...values.slice(0, -1)]);
            }
            await conn.commit();
            return { success: true, message: 'Updated', statusCode: 200 };
        }
    ...
    async deleteExitdetails(req, res) {
        let conn = null;
        try {
            const { employee_id } = req.body;
            if (!employee_id)
            ...
            conn = await database_1.pool.getConnection();
            const [result] = await conn.query('DELETE FROM exit_details WHERE employee_id = ?', [employee_id]);
            return {
                success: true,
                message: result.affectedRows > 0 ? 'Deleted' : 'Not found',
                statusCode: 200,
    ...
    async viewFamilyInfo(req, res) {
        ...
        try {
            ...
            conn = await database_1.pool.getConnection();
            const [rows] = await conn.query('SELECT * FROM family_info WHERE employee_id = ?', [employee_id]);
            return {
                success: true,
                message: rows.length > 0 ? 'Found' : 'Not found',
                data: rows[0] || null,
    ...
    async editFamilyInfo(req, res) {
        let conn = null;
        try {
            const { employee_id, father_name, mother_name, spouse_name, children_names, } = req.body;
            if (!employee_id)
                return {
                    success: false,
                    message: 'employee_id required',
            ...
            conn = await database_1.pool.getConnection();
            await conn.beginTransaction();
            const [result] = await conn.query(`UPDATE family_info SET father_name = ?, mother_name = ?, spouse_name = ?, children_names = ?
         WHERE employee_id = ?`, [
                father_name ?? null,
                mother_name ?? null,
                spouse_name ?? null,
            ...
            if (result.affectedRows === 0) {
                await conn.query(`INSERT INTO family_info (employee_id, father_name, mother_name, spouse_name, children_names)
           VALUES (?, ?, ?, ?, ?)`, [
                    employee_id,
                    father_name ?? null,
                    mother_name ?? null,
    ...
    async deleteFamilyInfo(req, res) {
        let conn = null;
        try {
            const { employee_id } = req.body;
            if (!employee_id)
            ...
            conn = await database_1.pool.getConnection();
            const [result] = await conn.query('DELETE FROM family_info WHERE employee_id = ?', [employee_id]);
            return {
                success: true,
                message: result.affectedRows > 0 ? 'Deleted' : 'Not found',
                statusCode: 200,
    ...
    async viewStatutoryInfo(req, res) {
        let conn = null;
        try {
            const { employee_id } = req.query;
            if (!employee_id)
            ...
            conn = await database_1.pool.getConnection();
            const [rows] = await conn.query('SELECT * FROM statutory_info WHERE employee_id = ?', [employee_id]);
            return {
                success: true,
                message: rows.length > 0 ? 'Found' : 'Not found',
                data: rows[0] || null,
    ...
    async editStatutoryInfo(req, res) {
        let conn = null;
        try {
            const { employee_id, pan_number, aadhaar_number, pf_number, uan_number } = req.body;
            if (!employee_id)
                return {
                    success: false,
                    message: 'employee_id required',
            ...
            conn = await database_1.pool.getConnection();
            await conn.beginTransaction();
            const [result] = await conn.query(`UPDATE statutory_info SET pan_number = ?, aadhaar_number = ?, pf_number = ?, uan_number = ?
         WHERE employee_id = ?`, [
                pan_number ?? null,
                aadhaar_number ?? null,
                pf_number ?? null,
            ...
            if (result.affectedRows === 0) {
                await conn.query(`INSERT INTO statutory_info (employee_id, pan_number, aadhaar_number, pf_number, uan_number)
           VALUES (?, ?, ?, ?, ?)`, [
                    employee_id,
                    pan_number ?? null,
                    aadhaar_number ?? null,
    ...
    async deleteStatutoryInfo(req, res) {
        let conn = null;
        try {
            const { employee_id } = req.body;
            if (!employee_id)
            ...
            conn = await database_1.pool.getConnection();
            const [result] = await conn.query('DELETE FROM statutory_info WHERE employee_id = ?', [employee_id]);
            return {
                success: true,
                message: result.affectedRows > 0 ? 'Deleted' : 'Not found',
                statusCode: 200,
    ...
    parseNoticePeriod(text) {
        if (!text)
            return null;
        const match = String(text).match(/(\d+)/);
        return match ? parseInt(match[1], 10) : null;
    }
    resolveReportingManager(value) {
    ...
    async getEmployeeIdByNumber(conn, employeeNumber) {
        const [rows] = await conn.query('SELECT employee_id FROM employees WHERE employee_number = ? LIMIT 1', [employeeNumber]);
