...
router.post('/offer-details', async (req, res) => {
    ...
    try {
        ...
        const [personal] = await database_1.pool.query(`SELECT candidate_id FROM hrms_master_data.personal_details WHERE Email = ?`, [Email]);
        ...
        const [candidateCheck] = await database_1.pool.query(`SELECT id FROM hrms_master_data.candidates WHERE id = ?`, [candidate_id]);
        ...
        const [result] = await database_1.pool.query(`INSERT INTO hrms_master_data.offer_details (Candidate_id, JoiningDate, OfferValidity)
             VALUES (?, ?, ?)`, [candidate_id, formattedJoiningDate, validityDays]);
